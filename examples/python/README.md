...
| `ex03_nb03_interactive_playground.ipynb` | `moveit_py` | **[노트북]** 라이브 놀이터 — 관절 헬퍼(`go_named/go_joints/grip`) + **Cartesian(EEF) 헬퍼(`go_pose/nudge/current_pose`)** + 상태 조회 + 다중 파이프라인 |
...
| `ex03_nb03_interactive_playground.ipynb` | `go_named/go_joints/grip` 관절 헬퍼 + **Cartesian(EEF) 헬퍼 `go_pose/nudge/current_pose`** 로 그리퍼 끝단 기준 포즈 조종, 현재 관절·tcp 포즈 조회, 다중 파이프라인 |

> 🎯 **그리퍼 끝단(EEF) TCP**: `moveit_py_params.py` 의 `tcp_offset_xyz="0.0 0.0 0.1425"` 로 `tcp_link` 를
> `link6`(플랜지)에서 +z 0.1425m 앞(그리퍼 파지 끝단)에 배치했습니다. 그래서 `pose_link="tcp_link"` 로 주는
> `go_pose` 좌표·RViz `tcp_link` 프레임·FK 가 모두 **그리퍼 끝단** 기준입니다. 이 값은 런치가 읽으므로 바꾸면
> **1회 리런치** 필요. 손끝보다 안쪽 파지점은 0.10~0.12 로 줄이면 됩니다.
